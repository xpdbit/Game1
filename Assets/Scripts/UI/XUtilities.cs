...
namespace XUtilities
{
    ...
    public class XUniTaskLoopProgress
    {
        ...
        public bool Tick()
        {
            ...
            float deltaTime = getDeltaTime?.Invoke() ?? Time.deltaTime;
...
namespace XUtilities
{
    ...
    {
        public static string FillNumberWithZero(string input, int totalLength = 4)
        {
            // 简单实现：提取字符串中的数字并补零
            var numStr = new string(input.Where(char.IsDigit).ToArray());
            if (int.TryParse(numStr, out int num))
            {
                return num.ToString().PadLeft(totalLength, '0');
            }
            return input;
        }
    }
